...
$sql = "SELECT * FROM tbl_employees WHERE UserId = '$UserId'";
$result = $db->query($sql);
...
//Total number of Annual leave taken in the current year
$sql = "select SUM(AppliedLeaves) AS TotalAnnualLeaveTaken from tbl_leave where UserId='2' AND YEAR(AppliedDate) = YEAR(CURRENT_DATE) AND LeaveStatus='Approved' AND LeaveType='2';";
$result = $db->query($sql);
$row = $result->fetch_assoc();
$TotalAnnualLeaveTaken = $row['TotalAnnualLeaveTaken'];

//Total number of casual leave taken in the current year
$sql = "select SUM(AppliedLeaves) AS TotalCasualLeaveTaken from tbl_leave where UserId='2' AND YEAR(AppliedDate) = YEAR(CURRENT_DATE) AND LeaveStatus='Approved' AND LeaveType='1';";
$result = $db->query($sql);
$row = $result->fetch_assoc();
$TotalCasualLeaveTaken = $row['TotalCasualLeaveTaken'];


if ($leaveEntries > 0) {
    $_SESSION['RemainingAnnualLeave'] = $TotalAnnualLeave- $TotalAnnualLeaveTaken;
    $_SESSION['RemainingCasualLeave'] = $TotalCasualLeave- $TotalCasualLeaveTaken;
} else {
    //if its first time leave Retrive the annual and casual leave from tbl_employees
