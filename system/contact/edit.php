...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    ...
    if ($_SERVER['REQUEST_METHOD'] == "GET") {
        extract($_GET);
        $db = dbConnection();
        $sql = "SELECT* FROM tbl_saloon_contact WHERE ContactId='$ContactId'";
        $result = $db->query($sql);

        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $sPhoneNumber = $row['PhoneNumber'];
                $sEmail = $row['Email'];
                $sAddressLine = $row['AddressLine'];
                $sCity = $row['City'];
            }
        }
    }
...
//01.check the request method
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        if (empty($message)) {
            ...
            $db = dbConnection();
            //echo 'Connected..!';
            $sql = "UPDATE tbl_saloon_contact SET PhoneNumber ='$sPhoneNumber',Email='$sEmail',AddressLine='$sAddressLine',City='$sCity'";
