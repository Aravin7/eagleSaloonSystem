...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    ...
    if ($_SERVER['REQUEST_METHOD'] == "GET") {
        extract($_GET);
        $db = dbConnection();
        $sql = "SELECT* FROM tbl_suppliers WHERE SupplierId='$SupplierId'";
        $result = $db->query($sql);
        
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $sName = $row['SupplierName'];
                $sPhoneNumber = $row['SupplierEmail'];
                $sEmail = $row['SupplierPhone'];
            }
        }
    }
...
//01.check the request method
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        if (empty($message)) {
            ...
            $sql = "UPDATE tbl_suppliers SET SupplierName='$sName',SupplierEmail='$sPhoneNumber',SupplierPhone='$sEmail' WHERE SupplierId='$SupplierId'";
